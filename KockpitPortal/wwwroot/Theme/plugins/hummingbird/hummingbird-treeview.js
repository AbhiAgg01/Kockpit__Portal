...
(function ($) {
	...
	$.fn.hummingbird.checkNode = function (tree, attr, name) {
		if (attr == "text") {
			...
			//if ($("#" + name + "").is(':disabled')) {
			//	$.fn.hummingbird.enableNode($(this), attr, name, true, true);
			//	tree.find('input:checkbox:not(:checked)[' + attr + '=' + name + ']').prop("indeterminate", false).trigger("click");
			//	$.fn.hummingbird.disableNode($(this), attr, name, true, false);
			//} else {
			//	tree.find('#' + name).prop("indeterminate", false).trigger("click");
			//}
			//tree.find('#' + name).prop("indeterminate", false).trigger("click");
			tree.find('input:checkbox:not(:checked)[' + attr + '=' + name + ']').prop("indeterminate", false).trigger("click");
		}else {
	...
	$.fn.hummingbird.getChecked = function (tree, list, onlyEndNodes, onlyParents, fromThis) {
		...
		if (onlyEndNodes == true) {
			activeGroup.find('input:checkbox.hummingbird-end-node:checked').each(function () {
				list.text.push($(this).parent("label").parent("li").text().trim());
			...
			if (onlyParents == true) {
				...
				activeGroup.find('input:checkbox:checked').each(function () {
					var liCollection = $(this).parent("label").parent("li").parent("ul").parent("li");
	...
	$.fn.hummingbird.ThreeStateLogic = function (tree, doubleMode, allVariables, checkDoubles, checkDisabled) {
		tree.find('input:checkbox').on('click', function (e) {
			...
			//console.log("this");
			//console.log($(this));
			var thisParentLabel = $(this).parent("label")
			if ($(this).prop("checked")) {
				var state = true;
				var checkSiblings = "input:checkbox:not(:checked)";

				//Mahesh
				$(thisParentLabel).attr("style", "color:green");
				$(thisParentLabel).children("span").attr("style", "color:green");
				//fire event
				...
				//Mahesh
				$(thisParentLabel).attr("style", "color:#ca2525");
				$(thisParentLabel).children("span").attr("style", "color:#ca2525");
				//fire event
			...
			var thisParentLabelCheckBox = $(this).parent("label").parent().parents("li").children("label").children("input:checkbox");
			$(thisParentLabelCheckBox).prop("indeterminate", true);
			$(thisParentLabelCheckBox).prop("checked", false);

			/*Mahesh*/
			nodes_below.parent("label").attr("style", state == true ? "color:green" : "color:#ca2525");
			if ($(this).hasClass("hummingbirdNoParent")) {
				var ele = $(this).parent("label").parent("li").parent("ul").children("li");
				var Childerlength = $(ele).length;
				var checkedChilderlength = $(ele).find("input:checkbox:checked").length;
				if (Childerlength > 0) {
					if (Childerlength === checkedChilderlength) {
						$(this).parent("label").parent().parents("li").children("label").children("span").attr("style", "color:green");
					} else if (checkedChilderlength < Childerlength && checkedChilderlength > 0) {
						$(this).parent("label").parent().parents("li").children("label").children("span").attr("style", "color:blue");
					} else if (checkedChilderlength == 0) {
						$(this).parent("label").parent().parents("li").children("label").children("span").attr("style", "color:#ca2525");
					}
				}
            }
			/*Mahesh*/
			...
			$(this).parent("label").siblings("ul").find("ul").map(function () {
				...
				var thisLiLabel = $(this).children("li").children("label");

				var disabled_sum_children = $(thisLiLabel).children("input:checkbox:disabled").length;
				...
				var checked_sum_children = $(thisLiLabel).children("input:checkbox:checked").length;
				var unchecked_sum_children = $(thisLiLabel).children("input:checkbox:not(:checked)").length;
			...
			if ($(clickedNode).attr('data-nodetype') == 'p') {
				checkList(clickedNode[0]);
				//console.log('checkList');
			} else {
				checkColumn(clickedNode[0]);
